...
2. [Sending 10,000 DOPLR to new wallet addresses.](#option2)
...
### Send 10,000 DOPLR to New Addresses

Just like setting up a standard MN. Send exactly 10,000 DOPLR to each new address created above.
...
Copy the masternode private key and correspondig collateral output transaction that holds the 1K DOPLR.

The masternode private key may be an existing key from [Option 1](#option1), or a newly generated key from [Option 2](#option2). 

*Please note, the masternode priviate key is not the same as a wallet private key. Never put your wallet private key in the masternode.conf file. That is equivalent to putting your 10,000 DOPLR on the remote server and defeats the purpose of a hot/cold setup.*
...
[From the doplr github repo](https://github.com/doplr-crypto/doplr/blob/master/doc/masternode_conf.md)
...
## What about the doplr.conf file?

If you are using a masternode.conf file you no longer need the doplr.conf file. The exception is if you need custom settings (thanks oblox). 

## Update doplr.conf on server

If you generated a new masternode private key, you will need to update the remote doplr.conf files.

Shut down the daemon and then edit the file.

```sudo nano .doplr/doplr.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the masternodeprivkey value in your remote doplr.conf file.
